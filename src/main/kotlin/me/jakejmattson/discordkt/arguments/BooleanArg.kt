 ...
 */
public open class BooleanArg(
    override val name: String = "Boolean",
    final override val truthValue: String = "true",
    final override val falseValue: String = "false",
    override val description: String = internalLocale.booleanArgDescription.inject(truthValue, falseValue)
    ...
    init {
        require(truthValue.isNotEmpty() && falseValue.isNotEmpty()) {
            "Custom BooleanArg ($name) options cannot be empty!"
        }

        require(!truthValue.equals(falseValue, ignoreCase = true)) {
            "Custom BooleanArg ($name) options cannot be the same!"
        }
    }
}