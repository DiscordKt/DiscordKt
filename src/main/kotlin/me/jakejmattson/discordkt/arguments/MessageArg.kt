...
public open class MessageArg(override val name: String = "Message",
    ...
    override suspend fun transform(input: String, context: DiscordContext): ArgumentResult<Message> {
        ...
        val message = when {
            ...
            else -> input.toSnowflakeOrNull()?.let { context.channel.getMessageOrNull(it) } ?: return Error("Invalid ID")
        }

        return Success(message)
    }

    override suspend fun generateExamples(context: DiscordContext): List<String> = listOf(context.message!!.id.toString())
}